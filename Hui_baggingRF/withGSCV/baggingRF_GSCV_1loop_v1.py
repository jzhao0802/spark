...
"""
This application is to do bagging RF with CV and grid search in ML module for 1 simulation
Also use Spark-csv package to read in CSV file.

please change line 42 to line 51 to your own variables
...
resultDir_s3 = path + "/02_result/" + app_name + '_' + st + "/"
...
#function 4: bagging random forest
...
def main(sc, pos_ori, neg_ori):

    #random split in positive cases and use HAE patient ID to select Non-HAE
    (pos_tr, pos_ts) = pos_ori.randomSplit([(1-ts_prop), ts_prop], seed=seed)
...
if __name__ == "__main__":
    ...
    avg_pred_ts.write.format("com.databricks.spark.csv")\
        .save((resultDir_s3 +"avg_pred_ts.csv"),header="true")
