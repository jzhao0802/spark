...
0. What the script does: This script serves as a template for using linear regression with the elastic-net penalty and an outer cross-evaluation plus an inner cross-validation for hyper-parameter selection. Both the outer cross-evaluation loop and the inner cross-validation loop use predefined fold IDs included in the input data. Using the template, it is possible to obtain the following outputs: 
...
sudo spark-submit --deploy-mode client --master yarn --num-executors 5 --executor-cores 16 --executor-memory 19g --py-files /path/to/imspacv.py /path/to/templateLinearRegression.py
...
2.2.1. Arguments for other functions such as initialising RegressionEvaluator, etc. Please refer to https://spark.apache.org/docs/2.0.0/api/python/pyspark.ml.html for details. 
